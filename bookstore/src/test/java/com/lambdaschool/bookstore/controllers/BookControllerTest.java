...
public class BookControllerTest
{
            ...
            Exception
    {
        ...
        b1.setBookid(1);
        b1.setTitle("Java");
        b1.setIsbn("9780738206720");

        ObjectMapper mapper = new ObjectMapper();

        String useString = mapper.writeValueAsString(b1);

        Mockito.when(bookService.save(any(Book.class))).thenReturn(b1);

        RequestBuilder rb = MockMvcRequestBuilders.post(apiUrl).contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON).content(useString);
